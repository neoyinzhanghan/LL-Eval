...
def get_diff(df):
    diff_cols = ['blasts', 'blast-equivalents', 'promyelocytes', 'myelocytes', 'metamyelocytes',
                 'neutrophils/bands', 'monocytes', 'eosinophils', 'erythroid precursors',
...
def get_cbc(df):
    cbc_cols = ['WBC', 'RBC', 'HGB', 'HCT', 'MCV', 'MCH',
                'MCHC', 'RDW', 'Platelets', 'Blast', 'Neutrophil', 'Mono', 'Eos',
                'Baso', 'Immature Granulocyte', 'Lymph', 'Plasma Cell', 'Nucleated RBC',
                'Abs Neut', 'Abs Mono', 'Absolute Eosinophil', 'Absolute Basophil',
                'Absolute Immature', 'Abs Lymph', 'Abnormal Lymph.*', 'Hairy Cells',
                'Other.*']
    conn = sqlite3.connect("/media/ssd2/clinical_text_data/Copath Database/copath.db")
